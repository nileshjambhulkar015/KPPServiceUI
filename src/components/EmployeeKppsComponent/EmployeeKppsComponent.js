...
export default function EmployeeKppComponent() {
    ...
    const [remark, setRemark] = useState('');
    const [kppResponses, setKppResponses] = useState([])
    const [todos, setTodos] = useState([{ empId: Cookies.get('empId'), empEId: Cookies.get('empEId'), roleId: Cookies.get('roleId'), deptId: Cookies.get('deptId'), desigId: Cookies.get('desigId'), ekppAchivedWeight: 0, ekppOverallAchieve: 0, ekppOverallTaskComp: 0, ekppAppliedDate: "", ekppEvidence: "", ekppStatus: "Pending" }]);
    useEffect(() => {
        EmployeeKppsService.getKPPDetails().then((res) => {
            setKppResponses(res.data);
    ...
    const handleTodoChange = (e, i, kppId, kppOverallTarget) => {
        console.log("kppOverallTarget", kppOverallTarget)
        const field = e.target.name;
        const newTodos = [...todos];
